...
手作業での編集は工数が多く、変更漏れや誤配送のリスクもありました。
...
楽天市場で購入した注文履歴を取得し、スプレッドシートに自動記録するスクリプト。

#### 背景

スプレッドシートに楽天市場の仕入れ情報を転記する作業に時間がかかっていました。
ショップ名、受注番号、件数の確認や割引や合計金額の集計など確認項目が多く、
重複確認や入力ミス防止が課題となっておりました。

#### 主な機能

- スクレイピングにより受注番号・日付・商品情報・割引率・送料・ポイントの使用有無など購入履歴を取得
- スプレッドシート上の過去データと照合し、既に記録された注文はスキップ
- 整形した購入記録をログ出力し、スプレッドシートへ記録

#### 効果

- 仕入れ情報の記録作業を完全自動化による時間短縮
- 目視による重複確認や手動による表記違いの修正作業が不要
...
Yahooメールに届いたAmazonのキャンセル依頼メールを自動チェックして、  
一括商品管理サービスGoQで該当の注文を検索してキャンセル処理。
その後に在庫連携を開いて在庫を増やして更新するスクリプト。
